...
`/azure-design <topic>` where topic is a kebab-case name like `hub-and-spoke`, `private-link-dns`, `aks-workload-identity-migration`, `aks-outbound-redesign`.
...
- `mcp_microsoft_doc_*` — fetch CAF/WAF references, reference architectures, Private Link/DNS/Workload Identity docs. Every citation must be live-fetched in this turn.
- `mcp_azure_*` — residual live checks where the design depends on current state (e.g., available regions for a paired-region hub, current AKS networking mode).

## Procedure

1. Load and validate config.
2. Locate the relevant inventory snapshot(s). If missing, stop and prompt the operator to run `/azure-inventory` first.
3. Fetch MS Learn references for the topic via `mcp_microsoft_doc_*`. Record every URL fetched and the section quoted.
