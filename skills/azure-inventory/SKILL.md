...
If `mcp_azure_*` is unavailable: stop, report which call failed, ask operator to start the MCP. Per Ground Rule 4.
...
7. Save to `{docs.spec_dir}/1.x-inventory/{YYYY-MM-DD}-{scope}.md`.
8. If `work_tracker` is configured, update the corresponding work item to reflect the inventory snapshot's existence (add a comment with the snapshot path).
9. Emit the closing line.
